...
a. seq(1, 3, by=0.2)
specify step size - The step size is by 0.2s.
...
```
Describe the output.
   - The output displays the "first" and "third" character horizontally and it create another row under it with the respective values of the characters inputted.
...
```
Describe the output.
   - The 2nd element of sequence x or the -2 modifies or changes into 0.
...
```{r, vector = TRUE}
data_rivers <- c(length(rivers), sum(rivers), mean(rivers), median(rivers), var(rivers),sd(rivers), min(rivers), max(rivers))
```
...
c. Interpret the data.
    - The data shows that the Power Ranking and Pay of a Celebrity named J.K. Rowling was modified. Its power ranking was changed, and the sequence of power ranking of the table became disordered given that the power ranking of J.K. Rowling which is 19 was changed into 15 as well its pay from 75 to 90.
